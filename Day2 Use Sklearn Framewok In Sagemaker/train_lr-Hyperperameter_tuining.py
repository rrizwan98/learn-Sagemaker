...
def model_fn(model_dir):
    model = joblib.load(os.path.join(model_dir, 'lr_model.joblib'))
    return model
...
if __name__ == '__main__':
    ...
    param_grid ={'C' : np.logspace(4, 20),
                 'solver' : ['lbfgs','saga'],
                 'max_iter' : [10, 50]}
    
    cls = LogisticRegression(random_state=random_state)  
    g_search = GridSearchCV(cls, param_grid = param_grid,cv = 2, n_jobs = 1, verbose = 0, return_train_score=True)
    ...
    model = os.path.join(model_dir, 'lr_model.joblib')
